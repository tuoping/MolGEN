...
out_dir = f"experiments/smallcell_SiO2_nvt_nowrap/kspring8/1600K1GPa/test_coesite_x0varkBT/priors"
...
args.data_dir = "data/SiO2/npt_1600K_1GPa/npt_coesite_dense/npt/"
...
def rollout(model, batch):
    logp, positions, _ = model.dump_priors(batch)
    new_batch = {**batch}
    ...
    for i_sample in range(1):
        ...
        if model.transport.latt_path:
            if args.likelihood is None:
                all_pred_frac_pos, _, all_cell  = model.dump_priors(batch)
            else:
                raise Exception("Not verified")
                logp, all_pred, _, zs = model.dump_priors(batch)
            ...
            if args.likelihood is None:
                all_pred_frac_pos, _  = model.dump_priors(batch)
            else:
                logp, all_pred_frac_pos, _, zs = model.dump_priors(batch)
