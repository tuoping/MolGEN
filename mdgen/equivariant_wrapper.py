...
from .model.leftnet import LEFTNet
from .model.equivariant_latent_model import LEFTNet_dpm
...
class EquivariantMDGenWrapper(Wrapper):
    def __init__(self, args):
        ...
        leftnet_config = dict(
            ...
            hidden_channels=196,
            num_radial=96,
            in_hidden_channels=args.num_species + 8,
            reflect_equiv=True,
            legacy=True,
            update=True,
            pos_grad=False,
            ...
            object_aware=True,
            out_scalar_channels=args.num_species + 8
        )
        leftnet = LEFTNet(**leftnet_config)
        self.model = LEFTNet_dpm(
            leftnet,
            cutoff=args.cutoff,
        ...
        if args.potential_model:
            self.potential_model = LEFTNet_dpm(
                leftnet,
                cutoff=args.cutoff,
                latent_dim=latent_dim,
                embed_dim=8,
    ...
    def prep_batch_x(self, batch):
        ...
        if (self.args.tps_condition and conditional_batch):
            ...
            return {
                ...
                'x0': (latents[:,0,...]*0.5+latents[:,-1,...]*0.5).unsqueeze(1).expand(B,T,L,3).to(_TORCH_FLOAT_PRECISION),
                'E': batch['e_now'].to(_TORCH_FLOAT_PRECISION),
                'loss_mask': batch["TKS_v_mask"]*cond_mask.unsqueeze(-1).to(_TORCH_FLOAT_PRECISION),
                'loss_mask_potential_model': torch.ones([B,T], device=latents.device).to(_TORCH_FLOAT_PRECISION),
                'model_kwargs': {
    ...
    def general_step(self, batch, stage='train'):
        ...
        if self.args.prediction == "velocity":
        ...
        if self.args.potential_model:
            ...
            loss_energy = ((torch.abs(energy -prep["E"]))*prep['loss_mask_potential_model']).sum(-1)
            self.prefix_log('loss_energy', loss_energy.mean(dim=-1))
            self.prefix_log('ref_energy', prep["E"].mean(dim=-1))
            self.prefix_log('pred_energy', energy.mean(dim=-1))
        ...
        if stage == "val":
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    # fragments_idx = prep['model_kwargs']['fragments_idx'][:,1].ravel().long().cpu().numpy()
                ...
                print("WARNING:: Inference failed")
